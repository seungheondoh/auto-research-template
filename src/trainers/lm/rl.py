"""GRPO (Group Relative Policy Optimization) RL fine-tuning trainer.
...
Batch keys:
    ...
    rewards [B, G]                             — scalar rewards per completion

The datamodule / reward_fn is expected to pre-compute rewards offline or online.
...
def _response_log_prob(model, prompt_ids, response_ids, response_mask) -> torch.Tensor:
    ...
    mask = torch.cat([
        torch.ones_like(prompt_ids), response_mask
    ], dim=1)
...
class RLTrainer(BaseTrainer):
    """GRPO policy optimisation over sampled completions.
