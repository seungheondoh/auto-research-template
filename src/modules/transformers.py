...
try:
    from torch.nn.attention.flex_attention import (
        BlockMask, _mask_mod_signature, flex_attention
    )
    _FLEX_ATTN = True
except ImportError:
    BlockMask = None
...
try:
    from xformers.ops import AttentionBias, fmha as _xformers_fmha
    _XFORMERS = True
except ImportError:
    AttentionBias = None
    _xformers_fmha = None
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if attn_impl == "flex_attention":
            if not _FLEX_ATTN:
                raise RuntimeError("flex_attention requested but torch>=2.5 not available; use attn_impl='sdpa'")
            ...
            if not _XFORMERS:
                raise RuntimeError("fmha requested but xformers not installed; use attn_impl='sdpa'")
            assert mask is None or isinstance(mask, AttentionBias)
            output = _xformers_fmha.memory_efficient_attention(xq, xk, xv, attn_bias=mask)
